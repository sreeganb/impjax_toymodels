...
\subsection{Accuracy: RMSD of the best-scoring models}

The ground truth of a toy model is the system itself as built, before the
shuffle: same representation, same copy number, rigid bodies on their input
coordinates. Accuracy is one RMSD per model to that structure, after a single
superposition over every rigid-body bead (\texttt{structure\_rmsd.py}; for
more than one copy, minimised over relabellings of identical copies). It is
reported for each run's $50$ best-scoring models: every frame of every RMF3
file the run wrote -- all replicas for replica exchange, the anneal and the
final population for SMC -- is re-scored by IMP on the CPU with the full
scoring function, and the lowest $50$ after a $25\%$ burn-in are kept
(\texttt{trajectory\_analysis.py}). Judging by best-scoring models asks the
question modelling asks: are the models one would pick by score correct?

The headline comparison is \emph{time to solution}: the wall time until a
run's best-scoring model is within $5$\,\AA{} of the ground truth. It folds
sampling efficiency and hardware into one number -- a GPU shortens it for a
population sampler directly, while a replica ladder only gains as far as its
number of MPI ranks allows. Replica exchange runs with one replica per MPI
rank (\texttt{run\_imp\_rex.py}, launched as \texttt{mpirun -np N}).
...
The earlier five-seed table used a final-frame window and CAPRI scores, both
since replaced by the measurement above; it will be re-measured with MPI
replica exchange ($8$ replicas) on the lab machine.
