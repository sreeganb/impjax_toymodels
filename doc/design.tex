...
\texttt{wrapper\_impjax.py} & \texttt{build\_log\_prob}, \texttt{run\_sampling} & all of the above, \texttt{rmh\_sampler} \\
\texttt{gpu\_io.py} & \texttt{DeviceBuffer}, \texttt{flush}, \texttt{write\_frame} & \texttt{state\_sync} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item Convergence diagnostics (BlackJAX's built-in tools) and the SMC
    (tempered) variant of this pipeline are intentionally deferred to
    separate, later, self-contained modules per \texttt{planning.md} --
    Section~\ref{sec:pipeline}'s algorithm already shows where an SMC
    variant plugs in without touching \texttt{proposals.py} or
    \texttt{state\_sync.py}.
\end{itemize}
